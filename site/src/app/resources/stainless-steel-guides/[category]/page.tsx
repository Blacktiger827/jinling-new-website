...
export function generateStaticParams() {
  return [...stainlessSteelGuideCategorySections, allStainlessSteelGuidesCategory].map((category) => ({
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `${current.title} | Jinling Metals`,
...
export default async function StainlessSteelGuideCategoryPage({
  params,
}: Props) {
  const { category } = await params;
  const current = getStainlessSteelGuidePageCategoryById(category);
  ...
  const allArticles = getArticlesBySlugOrder(
    articleSummaries,
    current.articleSlugs
  );
  const totalPages = Math.ceil(allArticles.length / STAINLESS_STEEL_GUIDES_PER_PAGE);
  const articles = allArticles.slice(0, STAINLESS_STEEL_GUIDES_PER_PAGE);
          ...
          <div className="mx-auto w-full max-w-[56rem] xl:col-start-2">
            <div className="grid w-full gap-5">
              {articles.length > 0 ? (
                ...
                <div className="border border-dashed border-cream-border bg-white/70 p-6 text-sm leading-7 text-text-secondary">
                  New stainless steel guide articles for this category will
